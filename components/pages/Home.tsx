...
import { HeroWorkspaceVisual } from '../BusinessVisuals';
    ...
    <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center w-full py-20">
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <Link href="/contact" className="bg-primary text-white px-8 py-4 font-headline font-bold text-lg hover:shadow-xl transition-all active:scale-95 cursor-pointer text-center">
            Request a Consultation
          </Link>
          <Link href="/services" className="bg-slate-200 text-primary px-8 py-4 font-headline font-bold text-lg hover:bg-slate-300 transition-all active:scale-95 cursor-pointer text-center">
            Explore Services
          </Link>
        </div>
      </motion.div>
    ...
    <div className="max-w-7xl mx-auto px-6">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
        ...
        <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-px bg-slate-200/20 ghost-border overflow-hidden">
          {[
            { icon: Globe, title: 'Website Design & Development', desc: 'Professional websites built with strong layouts, clean presentation and technology that supports long-term business use.', href: '/services/website-design-and-development' },
            { icon: Smartphone, title: 'Mobile App Development', desc: 'Consulting-led Android and iOS application development for startups, entrepreneurs and established businesses.', href: '/services/mobile-apps' },
            { icon: BarChart3, title: 'Digital Marketing', desc: 'SEO, PPC, email marketing, social media marketing and local business promotion designed to generate leads.', href: '/services/digital-marketing' },
            { icon: Database, title: 'ERP Solutions', desc: 'Custom ERP systems that simplify manual work, improve customer service and support business operations.', href: '/services/erp-solutions' },
            { icon: Gamepad2, title: 'Game Development', desc: 'Engaging interactive experiences built for multiple platforms including mobile, PC, and web.', href: '/services/game-development' },
          ].map((service, i) => (
            <Link key={i} href={service.href} className="bg-white p-10 hover:bg-slate-50 transition-colors group block">
  ...
  <section className="py-32 bg-slate-900 text-white">
    <div className="max-w-7xl mx-auto px-6 text-center">
      ...
      <div className="flex justify-center">
        <Link href="/contact" className="bg-secondary text-white px-12 py-5 font-headline font-bold text-xl hover:bg-secondary/90 transition-all active:scale-95 flex items-center gap-3 cursor-pointer">
