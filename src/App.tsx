...
const Logo = ({ className = "" }: { className?: string }) => (
  <div className={`flex items-center gap-2 ${className}`}>
    ...
    <div className="flex flex-col leading-none">
      <span className="text-lg font-black tracking-tighter text-[#D64545] font-headline">INTERNET</span>
...
const Navbar = () => {
  ...
  const services = [
    { title: 'Web Development', href: '/services', icon: Globe, desc: 'High-performance web systems.' },
    { title: 'Mobile Apps', href: '/mobile-apps', icon: Smartphone, desc: 'Native iOS & Android solutions.' },
    { title: 'Digital Marketing', href: '/digital-marketing', icon: BarChart3, desc: 'Algorithmic growth strategies.' },
    { title: 'ERP Solutions', href: '/erp-solutions', icon: Database, desc: 'Enterprise resource planning.' },
    { title: 'SaaS Lead Gen', href: '/saas-lead-gen', icon: Zap, desc: 'Specialized SaaS growth.' },
  ];
      ...
      <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled || isMobileMenuOpen ? 'bg-white/90 backdrop-blur-xl shadow-lg py-2' : 'bg-transparent py-4'}`}>
        <div className="flex items-center justify-between px-6 h-16 w-full max-w-7xl mx-auto">
          ...
          <div className="hidden md:flex items-center gap-8">
            <Link 
              to="/" 
              className={`font-bold font-headline transition-colors ${location.pathname === '/' ? 'text-secondary' : 'text-slate-500 hover:text-secondary'}`}
            >
              Home
            </Link>
            <Link 
              to="/about" 
              className={`font-bold font-headline transition-colors ${location.pathname === '/about' ? 'text-secondary' : 'text-slate-500 hover:text-secondary'}`}
            >
              About
            </Link>
                  ...
                  >
                    <div className="bg-white shadow-2xl border border-slate-100 p-8 grid grid-cols-2 gap-6 rounded-xl overflow-hidden">
                      <div className="col-span-2 mb-2">
                        <h4 className="text-[10px] font-bold uppercase tracking-widest text-slate-400 font-label">Diagnostic Protocols</h4>
                      </div>
                      {services.map((service, i) => (
                        <Link 
                          key={i} 
                          to={service.href}
                          className="flex items-start gap-4 p-3 hover:bg-slate-50 transition-colors rounded-lg group"
                        >
                      ...
                      <div className="col-span-2 mt-4 pt-4 border-t border-slate-50 flex justify-between items-center">
                        <Link to="/services" className="text-xs font-bold text-secondary hover:underline">View All Protocols →</Link>
                        <span className="text-[10px] text-slate-300 font-label uppercase tracking-widest">Clinical Precision</span>
                      </div>
                    </div>
                  </motion.div>
                )}
            ...
            <Link 
              to="/case-studies" 
              className={`font-bold font-headline transition-colors ${location.pathname === '/case-studies' ? 'text-secondary' : 'text-slate-500 hover:text-secondary'}`}
            >
              Results
            </Link>
            <Link 
              to="/careers" 
              className={`font-bold font-headline transition-colors ${location.pathname === '/careers' ? 'text-secondary' : 'text-slate-500 hover:text-secondary'}`}
            >
              Careers
            </Link>
            <Link 
              to="/contact" 
              className={`font-bold font-headline transition-colors ${location.pathname === '/contact' ? 'text-secondary' : 'text-slate-500 hover:text-secondary'}`}
            >
              Contact
            </Link>
          </div>
          ...
          <div className="flex items-center">
            ...
            <div className="md:hidden w-8 h-8 rounded-full bg-slate-200 flex items-center justify-center text-xs font-bold text-slate-600 ml-4">UP</div>
          </div>
        </div>
      </nav>
            ...
            >
              ...
              <div className="flex-grow overflow-y-auto p-6">
                <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      {navLinks.map((link, i) => (
                        <Link 
                          key={i} 
                          to={link.href}
                          className={`block text-2xl font-black font-headline tracking-tight ${location.pathname === link.href ? 'text-secondary' : 'text-slate-900'}`}
                        >
                          {link.name}
                        </Link>
                      ))}
                  ...
                  <div>
                    <h4 className="text-[10px] font-bold uppercase tracking-widest text-slate-400 font-label mb-4">Our Protocols</h4>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-12 mb-16">
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <p className="text-slate-400 text-sm leading-relaxed max-w-sm font-body">
            Clinical Precision in Digital Growth. We diagnose architectural failures and engineer high-performance solutions for high-growth tech companies.
          </p>
          <div className="flex items-center gap-3 py-3 px-4 bg-slate-900/50 border border-slate-800 rounded-sm w-fit">
            <div className="w-2 h-2 rounded-full bg-secondary animate-pulse"></div>
            <span className="text-[10px] font-bold font-label uppercase tracking-widest text-slate-300">System Status: Operational</span>
          </div>
        </div>
        ...
        <div className="lg:col-span-8 grid grid-cols-2 md:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            ...
            <ul className="space-y-3">
              <li><Link to="/services" className="text-sm text-slate-400 hover:text-white transition-colors font-body">Web Systems</Link></li>
              ...
              <li><Link to="/saas-lead-gen" className="text-sm text-slate-400 hover:text-white transition-colors font-body">SaaS Lead Gen</Link></li>
            </ul>
          </div>
          ...
          <div className="space-y-6 col-span-2 md:col-span-1">
            ...
            <ul className="space-y-3">
              <li><Link to="/case-studies" className="text-sm text-slate-400 hover:text-white transition-colors font-body">Case Studies</Link></li>
